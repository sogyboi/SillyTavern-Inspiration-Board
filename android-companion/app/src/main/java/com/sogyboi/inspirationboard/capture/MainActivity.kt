...
class MainActivity : Activity() {
    ...
    private val targets = listOf(
        Target("inbox", "Inbox"), Target("board", "Board"), Target("main", "Main portrait"),
        Target("face", "Face"), Target("hair", "Hair"), Target("body", "Body / pose"),
        Target("outfit", "Outfit"), Target("style", "Art style"), Target("mood", "Mood / vibe"),
        Target("environment", "Environment"), Target("generation", "Generation Studio"),
    )
    ...
    private fun configureWebView() {
        ...
        with(webView.settings) {
            ...
            userAgentString = "$userAgentString InspirationBoardCapture/0.5"
        }
    ...
    private fun handleIntent(intent: Intent, firstLaunch: Boolean) {
        ...
        if (!server.isNullOrBlank()) edit.putString("server", server.trimEnd('/'))
        ...
        val targetUrl = explicitUrl?.takeIf { it.startsWith("http://") || it.startsWith("https://") }
            ?: if (!firstLaunch) providerHomes[provider] else prefs.getString("lastUrl", null) ?: providerHomes[provider]
        if (!targetUrl.isNullOrBlank() && (webView.url == null || explicitUrl != null || !firstLaunch)) webView.loadUrl(targetUrl)
    ...
    private fun captureContext(preferredImage: String? = null, callback: (CaptureContext) -> Unit) {
        val script = """
            (function(){
        ...
        webView.evaluateJavascript(script) { raw ->
            val context = runCatching {
                ...
                CaptureContext(webView.url ?: "", webView.title ?: "Captured inspiration", preferredImage ?: "", "", prefs.getString("provider", "web") ?: "web")
            }
            callback(context)
        }
    }

    private fun openSaveSheet(context: CaptureContext) {
        ...
        content.addView(TextView(this).apply {
            text = "${context.title}\n${if (context.imageUrl.isNotBlank()) "Image detected · " else "Page link only · "}${context.provider.replaceFirstChar { it.uppercase() }}"
            textSize = 11f
        ...
        targets.chunked(2).forEach { rowTargets ->
            ...
            rowTargets.forEach { target ->
                ...
                row.addView(button, LinearLayout.LayoutParams(0, dp(48), 1f).apply { marginEnd = if (target == rowTargets.last()) 0 else dp(6); bottomMargin = dp(6) })
            }
        ...
        val fallback = Button(this).apply {
            ...
            setOnClickListener { dialog.dismiss(); shareFallback(context, prefs.getString("lastTarget", "inbox") ?: "inbox") }
        }
        content.addView(fallback, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(48)).apply { topMargin = dp(4) })
        dialog.show()
    }

    private fun captureNow(context: CaptureContext, target: String) {
        ...
        thread {
            try {
                postCapture(server, context, target)
                runOnUiThread { Toast.makeText(this, "Saved as ${targetLabel(target)} · return to Inspiration Board when ready", Toast.LENGTH_LONG).show() }
            } catch (error: Throwable) {
                runOnUiThread {
                    Toast.makeText(this, "Direct save failed: ${error.message}. Opening Android Share fallback.", Toast.LENGTH_LONG).show()
                    shareFallback(context, target)
                }
            }
        }
    ...
    private fun postCapture(server: String, context: CaptureContext, target: String) {
        val downloaded = if (context.imageUrl.startsWith("http://") || context.imageUrl.startsWith("https://")) runCatching { downloadImage(context.imageUrl) }.getOrNull() else null
        ...
        connection.inputStream.takeIf { code in 200..399 }?.close()
        connection.errorStream?.close()
        if (code !in 200..399) throw IllegalStateException("SillyTavern returned HTTP $code")
        connection.disconnect()
    }

    private fun buildMarker(context: CaptureContext, target: String): String {
    ...
    private fun writeFile(output: OutputStream, boundary: String, image: DownloadedImage) {
        output.write("--$boundary\r\nContent-Disposition: form-data; name=\"media\"; filename=\"${image.fileName}\"\r\nContent-Type: ${image.mime}\r\n\r\n".toByteArray())
        output.write(image.bytes)
        output.write("\r\n".toByteArray())
    }

    private fun downloadImage(value: String): DownloadedImage {
        ...
        val code = connection.responseCode
        if (code !in 200..299) throw IllegalStateException("Image returned HTTP $code")
        val mime = connection.contentType?.substringBefore(';')?.trim()?.takeIf { it.startsWith("image/") } ?: throw IllegalStateException("Selected URL is not an image")
        ...
        return DownloadedImage(bytes, mime, "capture-${System.currentTimeMillis()}.$extension")
    }
    ...
    private fun shareFallback(context: CaptureContext, target: String) {
    ...
    private fun showSettings(firstRun: Boolean = false) {
        ...
        content.addView(server, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(52)))
        content.addView(TextView(this).apply {
            val board = prefs.getString("boardName", "").orEmpty()
            text = if (board.isBlank()) "Open this app from Inspiration Board Capture Center to automatically link the current board." else "Linked board: $board"
            setTextColor(Color.rgb(170, 165, 187))
            textSize = 10f
            setPadding(0, dp(6), 0, 0)
        })
        AlertDialog.Builder(this)
            .setTitle(if (firstRun) "Connect Capture Browser" else "Capture Browser settings")
            .setView(content)
            .setPositiveButton("Save") { _, _ ->
                val value = server.text.toString().trim().trimEnd('/')
                prefs.edit().putString("server", value).apply()
                if (value.isNotBlank()) testServer(value)
            }
            .setNeutralButton("Open Capture Inbox") { _, _ ->
                val value = server.text.toString().trim().trimEnd('/')
                if (value.isNotBlank()) startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("$value/api/plugins/inspiration-board-sync/app/")))
            }
            .setNegativeButton("Cancel", null)
            .show()
    }
    ...
    private fun testServer(server: String) {
        thread {
            ...
            runOnUiThread { Toast.makeText(this, if (ok) "Capture bridge connected." else "Could not reach the capture bridge. Direct saves will fall back to Android Share.", Toast.LENGTH_LONG).show() }
        }
    }

    private fun targetLabel(id: String): String = targets.firstOrNull { it.id == id }?.label ?: id

    data class Target(val id: String, val label: String)
    data class CaptureContext(val pageUrl: String, val title: String, val imageUrl: String, val alt: String, val provider: String)
    data class DownloadedImage(val bytes: ByteArray, val mime: String, val fileName: String)
}
