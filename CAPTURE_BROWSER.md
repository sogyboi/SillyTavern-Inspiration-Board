# Android Capture Browser (v0.5.4)
...
1. Update the SillyTavern Inspiration Board extension to v0.5.4 and restart/reload SillyTavern.
2. Open **Inspiration Board → Capture**.
3. Tap **Install / update APK** in the Capture Browser banner.
4. Install `InspirationBoard-CaptureBrowser-v0.5.4.apk` from the GitHub release.
...
SillyTavern protects POST endpoints with CSRF validation. Capture Browser v0.5.4 now performs the same handshake that a SillyTavern browser page relies on:
...
This fixes the v0.5.0 native-save failure where the APK posted directly without a CSRF token. That failure also used to show only the endpoint URL because Android attempted to open the failed response as a successful input stream. v0.5.4 reads the correct success/error stream and reports the real HTTP status and response body.
...
v0.5.4 does not require a new server-plugin API. If `SillyTavern/plugins/inspiration-board-sync` from v0.4.0 is already installed and running, it remains compatible. The extension update and v0.5.4 APK are the new pieces.
