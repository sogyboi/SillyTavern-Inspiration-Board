...
export function installVeniceMediaBridge(router) {
  router.get('/venice/status', async (req, res) => {
    ...
    return res.json({ ok: true, configured, secretStorage: 'SillyTavern secrets.json', version: '0.6.1' });
  });
  ...
  router.post('/venice/image', async (req, res) => {
    ...
    try {
      ...
      if (references.length) {
        const editModel = cleanText(source.model, 300);
        const editBody = {
          prompt: cleanText(source.prompt, 32_768),
          safe_mode: source.safe_mode !== false,
        };
        ...
        if (references.length === 1) {
          response = await veniceFetch(key, '/image/edit', {
            method: 'POST',
            body: { ...editBody, model: editModel, image: stripDataUrl(references[0]) },
          });
        } else {
          response = await veniceFetch(key, '/image/multi-edit', {
            method: 'POST',
            body: { ...editBody, modelId: editModel, images: references.map(stripDataUrl) },
          });
        }
        if (!response.ok) {
          const payload = await responsePayload(response);
