...
index = replace_once(index, "import { installOpenRouterImagesBridge } from './openrouter-images-v58.mjs';\n", "import { installOpenRouterImagesBridge } from './openrouter-images-v58.mjs';\nimport { installVeniceMediaBridge } from './venice-media-v60.mjs';\n", 'Venice import')
index = replace_once(index, "const VERSION = '0.5.8';", "const VERSION = '0.6.0';", 'server version')
index = replace_once(index, "export async function init(router) {\n  installOpenRouterImagesBridge(router);\n", "export async function init(router) {\n  installOpenRouterImagesBridge(router);\n  installVeniceMediaBridge(router);\n", 'Venice init')
index = replace_once(index, "        capabilities: ['workspace-sync', 'android-share', 'remote-page-resolver', 'remote-image-proxy', 'native-json-capture', 'native-raw-capture', 'openrouter-image-api'],\n", "        capabilities: ['workspace-sync', 'android-share', 'remote-page-resolver', 'remote-image-proxy', 'native-json-capture', 'native-raw-capture', 'openrouter-image-api', 'venice-image-video'],\n", 'Venice capability')
index_path.write_text(index)


# Venice server bridge: structured reference routing and current video prompt limit.
bridge_path = Path('server-plugin/inspiration-board-sync/venice-media-v60.mjs')
bridge = bridge_path.read_text()
strip_anchor = """function stripDataUrl(value) {
...
"""
reference_helper = strip_anchor + r"""function normalizeReferenceEntries(value, max = 7) {
  ...
  if (total > MAX_REFERENCE_DATA_CHARS) {
...
"""
# The module calls this limit MAX_REFERENCE_DATA_CHARS.
bridge = replace_once(bridge, strip_anchor, reference_helper, 'structured reference helper')
bridge = replace_once(bridge, "    prompt: cleanText(source.prompt, 10_000),\n", "    prompt: cleanText(source.prompt, 2500),\n", 'video prompt limit')
routing = indent(dedent(r'''\
...
bridge = regex_once(
    bridge,
    r"  const refs = normalizeImageInputs\(source\.reference_images, 7\);\n  const model = payload\.model\.toLowerCase\(\);\n  if \(refs\.length\) \{.*?\n  \}\n  return payload;\n",
    routing,
    'R2V family routing',
)
...
# Venice client model capability / settings improvements.
ui_path = Path('venice-gen-v60.js')
ui = ui_path.read_text()
ui = regex_once(
    ui,
    r"export function veniceModelTask\(model\) \{.*?\n\}\n",
    dedent('''\
...
ui = replace_once(ui, "  const candidates = [payload?.balance?.usd, payload?.usd, payload?.balance, payload?.available_balance, payload?.available];\n", "  const candidates = [payload?.balances?.usd, payload?.balance?.usd, payload?.usd, payload?.balance, payload?.available_balance, payload?.available];\n", 'balance response')
persist_block = indent(dedent('''\
const persist = () => {
  const next = {
    ...settings,
    media,
...
'''), '  ')
ui = regex_once(ui, r"  const persist = \(\) => saveSettings\(\{.*?\n  \}\);\n", persist_block, 'live settings persistence')
ui = replace_once(ui, "    q('[data-v-audio-wrap]').hidden = media !== 'video' || !(c.audio || c.audio_configurable);\n", "    const supportsAudioConfig = c.audio_configurable === true || c.supportsAudioConfig === true;\n    q('[data-v-audio-wrap]').hidden = media !== 'video' || !supportsAudioConfig;\n", 'audio capability UI')
quote_block = indent(dedent('''\
const scheduleQuote = () => {
...
const loadCatalog'''), '  ')
ui = regex_once(ui, r"  const scheduleQuote = \(\) => \{.*?\n  \};\n\n  const loadCatalog", quote_block, 'video quote capability routing')
image_call = indent(dedent('''\
const modelAspects = listConstraint(model, 'aspectRatios', 'aspect_ratios');
const modelResolutions = listConstraint(model, 'resolutions');
...
'''), '        ')
ui = regex_once(ui, r"        const response = await api\('/venice/image', \{ method: 'POST', body: \{.*?\n        \} \}\);\n", image_call, 'image model sizing')
gen_quote = indent(dedent('''\
...
'''), '          ')
ui = regex_once(ui, r"          const quoteData = await \(await api\('/venice/video/quote'.*?\n", gen_quote, 'generate quote fields')
queue_call = indent(dedent('''\
const videoConstraints = constraints(model);
...
'''), '        ')
ui = regex_once(ui, r"        const queueData = await \(await api\('/venice/video/queue', \{ method: 'POST', body: \{.*?\n        \} \}\)\)\.json\(\);\n", queue_call, 'video reference queue fields')
...
package = replace_once(package, 'node --check launcher-v59.js &&', 'node --check launcher-v59.js && node --check launcher-v60.js &&', 'launcher check')
package = replace_once(package, 'node --check openrouter-gallery-v59.js &&', 'node --check openrouter-gallery-v59.js && node --check openrouter-browser-v60.js && node --check venice-gen-v60.js &&', 'client checks')
package = replace_once(package, 'node --check server-plugin/inspiration-board-sync/openrouter-images-v58.mjs &&', 'node --check server-plugin/inspiration-board-sync/openrouter-images-v58.mjs && node --check server-plugin/inspiration-board-sync/venice-media-v60.mjs &&', 'bridge check')
